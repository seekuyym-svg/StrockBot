...
def _set_today_decision(advice_text: str) -> bool:
    """
    更新 buy_decision_history.txt 中今天记录的 advice 列（三态：买股票/买ETF/空仓）
    用于在 execute_buy_calculation 实际分流后修正记录（如买股票日无票 → 空仓）
    ...
    try:
        ...
        for i, line in enumerate(lines[1:], 1):
            ...
            if len(parts) >= 14 and parts[0] == today_str:
                # advice 列在 index 13（date,weekday,...,final_total,advice,...）
                parts[13] = advice_text
                lines[i] = ','.join(parts) + '\n'
                updated = True
                logger.info(f"[HISTORY] 今日({today_str})决策更新为: {advice_text}")
                break
        if updated:
            with open(hist_file, 'w', encoding='utf-8') as f:
                f.writelines(lines)
            return True
        logger.warning(f"[HISTORY] 今日({today_str})无历史记录可更新")
        return False
    except Exception as e:
        logger.warning(f"[HISTORY] 更新今日决策失败: {e}")
        return False
...
def execute_buy_calculation():
    ...
    #    send_index_snapshot 现返回 (final_total, vol_ratio)
    final_total = None
    vol_ratio = None
    logger.info("\n[STEP 1] 计算大盘综合评分 + 国际市场早盘概览...")
    try:
        _res = send_index_snapshot()
        if isinstance(_res, tuple):
            final_total, vol_ratio = _res
        else:
            final_total = _res  # 兼容旧返回
    except Exception as e:
        logger.error(f"[STEP 1] 大盘评分计算异常: {e}")

    # 2. 定稿方案门控（2026-09验证，46天复利+32.52%）：
    #    评分 ∈ {3,4,6} 且 量比≤1.15 → 买13分动态股（前日score_stockpool_13输出池）
    #    其他评分 / 量比>1.15（过热）→ 买防守ETF
    #    无票日（评分3/4/6但无票）→ 空仓不买（收益0）
    #    （注意：评分7-9为过热陷阱、评分5为13分亏损日、评分2为弱市，均回避）
    #    评分3纳入：4个有票日13分+4.04% vs ETF-0.40%（样本少，观察中）
    defensive_mode = False  # 是否降级为防守ETF（非3/4/6窗口或过热）
    can_buy_13 = (final_total is not None and final_total in (3, 4, 6)
                  and (vol_ratio is None or vol_ratio <= 1.15))
    if can_buy_13:
        logger.info(f"\n[STEP 2] 评分{final_total}∈{{3,4,6}}且量比{vol_ratio}≤1.15 → 走13分股票池")
        # 选股来源：统一读 score_stockpool_13.py 输出的 stockpool_{昨日}.txt（动态13分池）
        # 注：offensive_stks 写死选股逻辑已废弃（generate_offensive_trade_file 不再调用），
        #     use_offensive_stocks 开关保留判断框架：=1 时也走同一股票池（仅记日志）
        use_offensive = 0
        try:
            from src.utils.config import get_config
        ...
        if use_offensive == 1:
            logger.info("[STEP 2] 检测到 use_offensive_stocks=1，但 offensive_stks 已废弃，统一走 score_stockpool_13 股票池")

        logger.info("\n[STEP 2] 检查选股结果文件...")
        if not check_stockpool_file():
            logger.warning("[SKIP] 前日选股个股为0，今日空仓")
            # 买股票日无票 → 空仓，并把历史记录从"买股票"修正为"空仓"
            _set_today_decision("空仓")
            # 推送空仓提示卡片（推送①早盘概览不变，仅此处买入委托卡片提示空仓）
            try:
                send_trade_notification(final_total_from_index=final_total, empty_position=True)
            except Exception as e:
                logger.error(f"[STEP 3] 空仓提示推送异常: {e}")
            return
        ...
        try:
            ...
            logger.error(f"[ERROR] 执行买入计算失败: {exc}")
            import traceback
            logger.error(traceback.format_exc())
            return
    else:
        # 转防守ETF（含评分None异常兜底）
        reason = f"评分{final_total}" if final_total is not None else "评分获取失败"
        if vol_ratio is not None and vol_ratio > 1.15:
            reason += f"且量比{vol_ratio}>1.15过热"
        logger.info(f"\n[STEP 2] {reason} → 使用防守ETF生成买入委托...")
        defensive_mode = True
        try:
            ...
            logger.error(f"[ERROR] 执行ETF买入计算失败: {exc}")
            import traceback
            logger.error(traceback.format_exc())
            _send_alert_notification("⚠️ 买入委托计算失败",
                                     f"**{reason}，改用防守ETF时执行异常**: {exc}")
            return
...
def send_index_snapshot():
    ...
    # 转为建议（10分制）—— 定稿方案门控口径（2026-09验证）：
    #   评分=3/4/6 → 可买13分动态股；评分=5 → 13分亏损日回避
    #   评分7-9 → 过热陷阱回避；评分2 → 弱市回避 → 均转防守ETF
    #   另：量比>1.15 → 过热熔断，即使3/4/6也转ETF
    if final_total in (3, 4, 6) and (vol_ratio is None or vol_ratio <= 1.15):
        advice = "🟢 买股票(优选13分)"
        advice_color = "green"
    elif final_total in (3, 4, 6) and vol_ratio is not None and vol_ratio > 1.15:
        advice = "🔴 量比过热→买ETF"
        advice_color = "red"
    else:
        advice = "🟡 买防守ETF"
    ...
    try:
        ...
        # 历史记录：只存二分类决策（买股票 / 买ETF），与门控条件一致
        if final_total is not None and final_total in (3, 4, 6) \
                and (vol_ratio is None or vol_ratio <= 1.15):
            advice_text = "买股票"
        else:
            advice_text = "买ETF"
    ...
    if not index_list:
        ...
        return final_total, vol_ratio  # 评分已算出，仍返回供Step2使用
...
def send_trade_notification(final_total_from_index=None, defensive_mode: bool = False,
    ...
    Args:
        ...
        empty_position: 空仓日（买股票窗口但前日选股0只）→ 仅提示空仓
    """
    try:
        # ── 空仓日（前日选股个股为0）：不读交易文件，直接推空仓提示卡片 ──
        if empty_position:
            notifier = get_feishu_notifier()
            if not notifier.enabled:
                ...
                return
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            content = f"**前日选股数为0，今日空仓。**\n"
            content += f"**时间**: {current_time}\n"
            logger.info("[TRADE] 空仓日（前日选股0只），推送空仓提示")
            message = {
                "msg_type": "interactive",
                "card": {
                    "config": {"wide_screen_mode": True},
            ...
            import json
            headers = {'Content-Type': 'application/json'}
            response = requests.post(notifier.webhook_url, headers=headers,
                                     data=json.dumps(message, ensure_ascii=False).encode('utf-8'), timeout=10)
            if response.status_code == 200:
                result = response.json()
                if result.get('StatusCode') == 0 or result.get('code') == 0:
                    logger.success("[OK] 空仓提示推送成功")
                else:
                    logger.error(f"[ERROR] 飞书API返回错误: {result.get('msg', '未知')}")
            else:
                logger.error(f"[ERROR] 空仓提示推送失败，HTTP状态码: {response.status_code}")
            return
        ...
        # 卡片颜色：defensive_mode 由 execute_buy_calculation 按定稿门控算出（权威）
        if defensive_mode:
            # 防守ETF：橙色提示（评分极低<4时红色）
