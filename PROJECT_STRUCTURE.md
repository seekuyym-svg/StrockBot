...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| v3.3.1 | 2026-05-15 | **新增交易日过滤功能**：资讯监控调度器现在仅在工作日执行，自动跳过周末和法定节假日；使用 akshare 获取A股交易日历并缓存，失败时降级为仅过滤周末；同时跳过选股结果推送，避免非交易日发送无效通知 |
| v3.2.0 | 2026-05-13 | **新增选股结果通知功能**：集成到资讯监控调度器，在每日资讯日报同一时间点自动推送当日选股结果；支持通过akshare获取真实交易日历，周末/节假日自动追溯到最近交易日；飞书通知包含股票名称、代码、评分、PE-TTM（滚动市盈率） |
| v3.1.0 | 2026-05-13 | **评分系统统一升级**：将资讯监控和飞书通知的评分系统从旧版（-6.0~+6.0）升级为新版百分制（0-100分），与回测模块保持一致；新增趋势映射规则（80-100:🟢强势多头，60-79:🟡温和上涨，40-59:⚪震荡整理，20-39:🟠弱势下跌，0-19:🔴极弱空头） |
