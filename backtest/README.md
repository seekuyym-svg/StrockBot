...
| 文件 | 位置 | 说明 |
|:----|:----|:------|
| `signal_history.json` | `data/` | 大盘环境信号历史记录（每日20:50生成） |
| `buy_decision_history.txt` | `data/` | 9:26早盘评分历史（13列，含量比/科创/KOSPI/沪深300分项） |
| `market_emotion.txt` | `data/` | 情绪开关数据（手动/脚本更新，21:00推送读取） |
| `index_kc.csv` | `data/` | 科创综指日K线数据（已不直接使用，由signal_history.json替代） |

**文件命名约定**：`.bak`=原始选股池，`.txt`=评分结果，`.txt+收益率`=回测结果

---

## 📅 每日推送流程

### 9:26 早盘推送（`buy_order_scheduler.py`）

```
① 三大外盘指数 → 🇯🇵日经225 🇰🇷KOSPI 🇺🇸纳斯达克
② 综合评分（10分制） → 决定是否买入
③ 情绪开关 → 涨停<35家熔断
```
...
| 维度 | 3分 | 2分 | 1分 | 0分 |
|:---|---:|:---:|:---:|:---:|
| **量比** | ≤1.00 | 1.01~1.10 | 1.11~1.15 | >1.15 |
| **前日科创50** | ≥+2% | 0~+2% | -2%~0 | <-2% |
| **当日KOSPI** | ≥+5% | +3%~+5% | 0~+3% | <0% |
| **沪深300健康** | — | — | — | ✅得1分，❌得0分 |

**过热降级**：前日科创50 >+3%扣1分，>+4%扣2分，>+5%扣3分

**建议阈值**：≥8=推荐买入，6~7=可买(控制仓位)，4~5=谨慎，<4=不买

### 21:00 晚间推送（`news_scheduler.py`）

```
① 大盘环境信号 → 含情绪开关（涨停/跌停/涨跌比/大盘评级）
② 持仓收益率通知 → T+2交易结算
③ 选股结果通知 → 当日评分结果
④ [已暂停] 资讯日报
```
...
| 工具 | 功能 |
|:----|:------|
| `backtest/analyze_potential.py` | 牛股潜质分析（偏离MA20+历史出镜率） |
| `backtest/analyze_market_relation.py` | 回测收益率与沪深成交量/科创50关系分析 |
| `backtest/analyze_overseas_relation.py` | 回测收益率与纳指/日经/KOSPI关系分析 |
| `backtest/fetch_market_emotion.py` | 同花顺情绪数据爬虫（playwright） |
| `trade_decision/update_indices.py` | 更新指数数据+生成大盘信号+情绪开关（20:50定时） |
| `trade_decision/market_signal.py` | 大盘环境信号逻辑（双指数共识） |
...
|:----|:---------|:---------|
| **纳斯达克** | 腾讯kline | 无降级 |
| **日经225** | 东方财富push2 → push2his日K(重试+日期检) → 新浪日K(日期检) |
| **KOSPI** | 东方财富push2 → push2his日K(重试+日期检) → 新浪日K(日期检) |

日经和KOSPI走同一套三层降链逻辑，各自独立获取。

---

## 🔑 关键结论（已记录到memories）

- **KOSPI与A股相关性**：75%方向一致率（12期样本），评分中权重较高
- **选股数量与胜率**：选股越多胜率越低，待后续优化
- **量比阈值**：>1.15时市场过热，应谨慎
- **纳指相关性**：与A股回报关系弱，KOSPI和科创50更具参考价值
